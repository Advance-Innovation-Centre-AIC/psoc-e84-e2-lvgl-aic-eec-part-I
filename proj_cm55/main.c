...
/*******************************************************************************
 * Course Example Selector
 * Change SELECTED_PART and SELECTED_EXAMPLE in example_selector.h to run
 * different examples.
 ******************************************************************************/
#include "example_selector.h"

/*******************************************************************************
 * IPC Communication (CM55 <-> CM33-NS)
 * Enable/disable IPC by setting IPC_ENABLED to 1 or 0
 ******************************************************************************/
...
static void cm55_gfx_task(void *arg)
{
    ...
    if (CY_GFX_SUCCESS == gfx_status)
    {
        ...
        if (VG_LITE_SUCCESS == vglite_status)
        {
             ...
             * Change SELECTED_PART and SELECTED_EXAMPLE in example_selector.h
             */
...
int main(void)
{
...
#if IPC_ENABLED
    /* Initialize IPC communication with CM33-NS */
    if (CY_IPC_PIPE_SUCCESS == cm55_ipc_init())
    {
        /* Create IPC receive task */
        cm55_ipc_create_task();
    }
#endif
    ...
    if (pdPASS == task_return)
    {
        printf("****************** "
               ...
               "****************** \r\n");
        printf("Embedded C for IoT Course - Part 1-5 Examples\r\n");
        printf("Edit example_selector.h to change examples\r\n\n");
#if IPC_ENABLED
        printf("IPC Communication: ENABLED\r\n\n");
#endif
